...
class groupLista(nn.Module):
    ...
    def __init__(self, params: ListaParams):
        ...
        if params.std_gamma:
            n_diag_g = num_lista-1 if params.multi_std else 1
            self.std_g = nn.ParameterList([nn.Parameter(torch.zeros(1, params.num_filters, 1, 1)) for _ in range(n_diag_g)])
    ...
    def forward(self, I, writer=None, epoch=None):
        params = self.params
